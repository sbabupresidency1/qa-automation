...

{
	...
	public static void selectRAAndroidEnvironments(AndroidDriver driver) 
	{
		...
		{
			WebElement KulfiEnvironmentName= driver.findElement(By.xpath(OR.ANDROID_RA_MEMBER_KULFI_ENVIRONMENT_NAME));
			verifyElementIsPresent(driver, KulfiEnvironmentName);
			click(KulfiEnvironmentName);
			wait(driver, "3");
		}

		else if(Directory.zado_environment.equalsIgnoreCase("ConfigurationFile-Test"))
		{
			WebElement TestEnvironmentName= driver.findElement(By.xpath(OR.ANDROID_RA_MEMBER_TEST_ENVIRONMENT_NAME));
			verifyElementIsPresent(driver, TestEnvironmentName);
			click(TestEnvironmentName);
			wait(driver, "3");
		}
